...
namespace NamNguyen\SpeedOptimizer\Plugin;
...
use NamNguyen\SpeedOptimizer\Helper\Data;
...
class SpeedOptimizer extends \Magento\Framework\View\Element\Template
{
    ...
    public function addLazyLoadJs( $content, $selector='img', $exclude='.loaded' )
    {
        ...
        $script = '<script type="text/javascript"> require(["jquery", "namnguyen/lazyload", "domReady!"], function($, lazyload){$(' . $selector .').not("' . $exclude . '").lazyload();});</script>';
    ...
    public function addLoading($content)
    {
        ...
        $loadingBody = $loadingBody ? $mediaUrl . 'namnguyen/speedoptimizer/' . $loadingBody : $placeholder;
        ...
        $loadingImg  = $loadingImg ?  $mediaUrl . 'namnguyen/speedoptimizer/' . $loadingImg : $placeholder;
